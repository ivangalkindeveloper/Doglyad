...
- `backend/main/secrets/` — секреты главного бэкенда (`.env`, `.env.<профиль>`, `firebase_credentials.json`, карта эндпоинтов): SMTP, Firebase, адреса GPU-виртуалок.
- `backend/inference/secrets/` — секреты сервиса инференса (`.env`, `firebase_credentials.json`): HF-токен, Firebase.

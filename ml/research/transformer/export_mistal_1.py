    ...
    def forward(
        ...
        **kwargs,
        ...
        if past_key_value is None:
            raise ValueError("past_key_value is required for slice-update attention.")
...
class StatefulMistralForCausalLM(torch.nn.Module):
    def __init__(self, model_path: str, max_context_size: int = 2048, batch_size: int = 1) -> None:
        ...
        # Подменяем внимание на кастомное с slice-обновлением KV cache
        for idx, layer in enumerate(self.model.model.layers):
            layer.self_attn = SliceUpdateMistralAttention(self.model.config, layer_idx=idx).to(
                dtype=torch.float16
            )
...
def export() -> None:
    ...
    mlmodel_int8 = ct.optimize.coreml.linear_quantize_weights(mlmodel_fp16, config=config)
    mlmodel_int8._spec.description.metadata.userDefined.update({METADATA_TOKENIZER: MODEL_ID})
    del mlmodel_fp16
    mlmodel_int8.save("StatefulMistral7BInstructInt8.mlpackage")

...
Источник правды по конфигурации — backend/config/runpod_endpoints.json.
...
АЛГОРИТМ (одинаковый для plan и apply):
    ...
    Шаг 5. Напечатать карту modelId -> URL для backend/secrets/runpod_endpoint.json.
...
# backend/scripts/runpod_sync.py -> backend/
...
def print_urls(urls: dict[str, str], write: bool) -> None:
    """Печатает JSON в формате backend/secrets/runpod_endpoint.json."""

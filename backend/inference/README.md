...
`app_check.py` здесь и в `backend/main` разошлись по смыслу, а не только по коду: там проверка включается конфигурацией (`LLM_MODE`, `ENVIRONMENT`), здесь она безусловна. Общего кода, который стоило бы вынести, между ними по сути и нет — совпадает только вызов `firebase_admin`.
